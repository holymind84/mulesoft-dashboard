...
function ByApplication({ selectedEnv }) {
  ...
  const fetchAllStoresData = async () => {
    ...
    try {
      const promises = availableStores.map(store => 
        fetch(
          `http://localhost:5000/api/objectstore/${store}?startDate=${statsForm.startDate}&endDate=${statsForm.endDate}&period=${statsForm.period}`,
          {
            headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedStore || !selectedEnv) return;
